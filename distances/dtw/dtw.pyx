...
def dynamic_time_warping(np.ndarray[double, ndim=1] x, np.ndarray[double, ndim=1] y , w = -1):
    # make sure x is shorter than y 
    # if not permute
    cdef np.ndarray[double, ndim=2] X = x.reshape(-1,1)
    cdef np.ndarray[double, ndim=2] Y = y.reshape(-1,1)
